...
from Music import dbb, app, BOT_USERNAME, BOT_ID, USERID, USERNAME, USERUSERNAME, USERMENTION
from Music.MusicUtilities.tgcallsrun import (music, convert, download, clear, get, is_empty, put, task_done, USER_ACC)
...
async def play(_, message: Message):
    ...
    if not a.can_invite_users:
        ...
        return
    try:
        b = await app.get_chat_member(message.chat.id , USERID) 
        if b.status == "kicked":
            await message.reply_text(f"❌ {USERNAME}(@{USERUSERNAME}) is banned in your chat **{chat_title}**\n\nUnban it first to use Music")
            return
    except UserNotParticipant:
        if message.chat.username:
            try: 
                await USER_ACC.join_chat(f"{message.chat.username}")
                await message.reply(f"✅ {USERNAME} Joined Successfully",) 
            ...
            try:
                ...
                await USER_ACC.join_chat(yxy.invite_link)
                await message.reply(f"✅ {USERNAME} Joined Successfully",) 
                await remove_active_chat(chat_id)
